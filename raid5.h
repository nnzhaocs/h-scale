...
struct stripe_head {
	...
	*/
	...
	struct r5dev {
		...
		unsigned long	flags;
		int disk_num; //by xp: the disk number this stripe lies
		int disk_offset; //by xp: 这个dev相对这个条带第1个dev的偏移(因为扩容后，同一stripe的各个dev相对所在磁盘的偏移是不一样的)
...
struct raid5_private_data {
	...
	struct disk_info	*disks; //by xp: type is struct disk_info *, equals to array[]
};
